...
优化版访问相同的 17 个覆盖点，但以内外圈交替顺序缩短移动路径；每个频道收集到 4 条有效方向后停止继续测量。清除阶段采用多起点最近邻和 2-opt 改良访问顺序，减少末段长距离折返：
...
演练结束后，以模拟器界面显示的干扰源总数作为最终验收值；`detected channels` 与 `cleared channels` 相同并不能排除漏检。可将总数和案例编码传给日志分析脚本：

```powershell
python analyze_logs.py --total-sources 15 --case-code <测试案例编码>
```

脚本会同时输出清除比例、程序运行时间和可填写到论文表 1 的一行数据。三次正式测试结束后，还需从模拟器导出加密行为日志，保持原文件名并放入支撑材料。
